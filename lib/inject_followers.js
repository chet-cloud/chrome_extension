...
// })
//
// API.call("save",{key:"name",value:"cc"},(response)=>{
//     console.log(response)
// })
//
// API.call("get",{key:"name"},(response)=>{
//     console.log(response)
// })
...
function getFollowersNames(){
    if(document.querySelector("body > div > div > div > div > ul > div") == null){
        return []
    }
    let a = document.querySelector("body > div > div > div > div > ul > div").querySelectorAll("a")
    if( a === null){
        return []
    }
...
WaitUntil(() => getFollowersEl() !== null, 1000 * 60).then(() => {
    ...
    followerNumber = parseInt(followerNumber)
    simulatedClick(clickTarget)
    WaitUntil(() => {
        return getFollowersNames().length === followerNumber
    }, 1000 * 60).then(() => {
